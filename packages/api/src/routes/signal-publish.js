...
exports.createSignalPublishRoutes = createSignalPublishRoutes;
const express_1 = require("express");
const signal_engine_1 = require("@agoraiq/signal-engine");
const auth_1 = require("../middleware/auth");
const validator = new signal_engine_1.SignalValidator();
const formatter = new signal_engine_1.SignalFormatter();
// ─────────────────────────────────────────────────────────────────────────────
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
function tradeToSignalFields(trade, template) {
    ...
    const targets = [trade.tp1Price, trade.tp2Price, trade.tp3Price]
        .map(Number)
    ...
    return {
        ...
        entries: [Number(trade.entryPrice)],
...
function exchangeEnumToDisplay(enumValue) {
    return EXCHANGE_MAP[enumValue] ?? enumValue.replace(/_/g, ' ');
}
// ─────────────────────────────────────────────────────────────────────────────
// Route factory
// ─────────────────────────────────────────────────────────────────────────────
function createSignalPublishRoutes(db) {
    ...
    router.post('/:tradeId/validate', async (req, res) => {
        try {
            ...
            const fields = tradeToSignalFields(trade, overrides);
    ...
    router.post('/:tradeId/preview', async (req, res) => {
        try {
            if (!requirePro(req, res))
                return;
            ...
            const fields = tradeToSignalFields(trade, template);
    ...
    router.post('/:tradeId/publish', async (req, res) => {
        try {
            if (!requirePro(req, res))
                return;
            const { channels, // { telegram?: string[], discord?: string[] }
            templateId, format = 'cornix', } = req.body;
            ...
            const cornixFormatted = formatter.formatCornix(fields);
            ...
            dispatchPublishJob(publish.id, channels, cornixFormatted.text, fields, db)
                .catch((err) => {
    ...
    router.post('/templates', async (req, res) => {
        try {
            if (!requirePro(req, res))
                return;
            const { name, exchange, leverage, footer, channels, isDefault = false, } = req.body;
...
// ─────────────────────────────────────────────────────────────────────────────
async function dispatchPublishJob(publishId, channels, cornixText, _fields, db) {
    const results = { telegram: [], discord: [] };
    // ── Telegram ──────────────────────────────────────────────────────────────
    ...
    if (channels.discord?.length) {
        ...
        const discordFormatted = formatter.formatDiscordEmbed(_fields);
...
//# sourceMappingURL=signal-publish.js.map