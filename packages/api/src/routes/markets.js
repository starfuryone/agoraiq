...
exports.createMarketsRoutes = createMarketsRoutes;
const express_1 = require("express");
const db_1 = require("@agoraiq/db");
const log = (0, db_1.createLogger)('markets-routes');
...
function createMarketsRoutes(db) {
    ...
    // ── GET /meta/assets ─────────────────────────────────────────────────────────
    router.get('/meta/assets', async (_req, res) => {
        try {
    ...
    router.get('/compare', async (req, res) => {
        try {
            ...
            const conditions = ["mp.status IN ('ONLINE','POST_ONLY','LIMIT_ONLY')"];
            const params = [];
            if (symbol) {
    ...
    router.post('/stats/refresh-view', async (_req, res) => {
        try {
            await db.$executeRawUnsafe(`REFRESH MATERIALIZED VIEW CONCURRENTLY market_stats_latest`);
    ...
    router.post('/stats', async (req, res) => {
        try {
            ...
            if (rows.length > 500)
                return res.status(400).json({ error: 'Max 500 rows per batch' });
            const values = [];
            const params = [];
            for (const r of rows) {
                if (!r.exchange || !r.pairId)
                    continue;
                const i = params.length;
                params.push(String(r.exchange).toUpperCase(), String(r.pairId), r.bid ?? null, r.ask ?? null, r.last ?? null, r.spreadAbs ?? null, r.spreadBps ?? null, r.volume24h ?? null, r.volume24hUsd ?? null, r.fundingRate ?? null, r.liquidityScore != null ? Math.round(r.liquidityScore) : null, r.volatilityScore != null ? Math.round(r.volatilityScore) : null, r.fetchLatencyMs ?? null);
                values.push(`($${i + 1},$${i + 2},NOW(),$${i + 3},$${i + 4},$${i + 5},$${i + 6},$${i + 7},$${i + 8},$${i + 9},$${i + 10},$${i + 11},$${i + 12},$${i + 13})`);
            }
            if (!values.length)
                return res.status(400).json({ error: 'No valid rows' });
            await db.$executeRawUnsafe(`INSERT INTO market_stats
    ...
    router.get('/', async (req, res) => {
        try {
            const pageSize = Math.min(parseInt(req.query.pageSize || '200', 10), 2000);
            const page = Math.max(parseInt(req.query.page || '1', 10), 1);
            const offset = (page - 1) * pageSize;
            const { exchanges, search, base, quote, status, marginAvailable, minVolume, maxSpreadBps, sort, } = req.query;
    ...
    router.get('/:exchange/:pairId', async (req, res) => {
        try {
            const exchange = req.params.exchange.toUpperCase();
            const pairId = req.params.pairId;
            const [pairRows, statRows, changelogRows, syncRows, historyRows] = await Promise.all([
                db.$queryRawUnsafe(`
