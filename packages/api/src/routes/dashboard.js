...
function createDashboardRoutes(db) {
    ...
    router.get('/signals', async (req, res) => {
        try {
            ...
            if (req.query.provider)
                where.providerKey = req.query.provider;
            if (req.query.symbol)
                where.symbol = req.query.symbol.toUpperCase();
            if (req.query.timeframe)
                where.timeframe = req.query.timeframe;
            if (req.query.action)
                where.action = req.query.action;
            if (req.query.minConfidence) {
                where.confidence = { gte: parseFloat(req.query.minConfidence) };
            }
    ...
    router.get('/signals/:id', async (req, res) => {
        try {
            ...
            const itbMeta = (meta && typeof meta === 'object') ? meta : {};
    ...
    router.get('/providers', async (req, res) => {
        try {
            ...
            const cached = cache_1.dashboardCache.get(cacheKey);
            if (cached) {
                res.json(cached);
                return;
            }
            ...
            const tradesByProvider = new Map();
            for (const t of allTrades) {
                if (!tradesByProvider.has(t.providerId))
                    tradesByProvider.set(t.providerId, []);
            ...
            cache_1.dashboardCache.set(cacheKey, result);
            res.json(result);
        }
        catch (err) {
    ...
    const WatchlistSchema = zod_1.z.object({
        type: zod_1.z.enum(['symbol', 'provider']),
    ...
    // ── GET /exports/csv ────────────────────────────────────────
    router.get('/exports/csv', async (req, res) => {
        try {
    ...
    router.get('/performance', async (req, res) => {
        try {
            ...
            const providerSlug = req.query.provider;
            let since;
            const now = new Date();
            if (period === 'today') {
            ...
            if (since)
                where.exitedAt = { ...where.exitedAt, gte: since };
            if (providerSlug) {
            ...
            for (const t of trades) {
                if (t.status === 'HIT_TP') {
                    ...
                    tempLose++;
                    tempWin = 0;
                    if (tempLose > maxLoseStreak)
                        maxLoseStreak = tempLose;
                }
            }
            ...
            const rBuckets = { '< -2R': 0, '-2R to -1R': 0, '-1R to 0R': 0, '0R to 1R': 0, '1R to 2R': 0, '2R to 3R': 0, '> 3R': 0 };
            ...
            for (const t of trades) {
                if (t.enteredAt && t.exitedAt) {
                    ...
                    if (ms > 0) {
                        ...
                        durations.push(hrs);
                        if (t.status === 'HIT_TP')
                            tpDurations.push(hrs);
                        if (t.status === 'HIT_SL')
                            slDurations.push(hrs);
                    }
                }
            }
    ...
    // ── POST /signal-log ──────────────────────────────────────
    router.post('/signal-log', async (req, res) => {
        try {
            const { symbol, direction, entryMin, entryMax, stopLoss, takeProfits, leverage, marketType, confidence, rawMessage, note } = req.body;
            if (!symbol) {
            ...
            await db.$executeRawUnsafe(`INSERT INTO "SignalLog" ("id","workspaceId","userId","symbol","direction","entryMin","entryMax","stopLoss","takeProfits","leverage","marketType","confidence","rawMessage","note","createdAt")
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10,$11,$12,$13,$14,NOW())`, id, req.user.workspaceId, req.user.userId, symbol, direction || null, entryMin || null, entryMax || null, stopLoss || null, JSON.stringify(takeProfits || []), leverage || null, marketType || null, confidence || null, rawMessage || null, note || null);
            res.json({ ok: true, id });
        }
        catch (err) {
    ...
    // ── GET /signal-log ────────────────────────────────────────
    router.get('/signal-log', async (req, res) => {
        try {
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 20;
            const skip = (page - 1) * limit;
            const [entries, total] = await Promise.all([
                db.$queryRawUnsafe(`SELECT * FROM "SignalLog" WHERE "workspaceId"=$1 ORDER BY "createdAt" DESC LIMIT $2 OFFSET $3`, req.user.workspaceId, limit, skip),
                db.$queryRawUnsafe(`SELECT COUNT(*)::int as count FROM "SignalLog" WHERE "workspaceId"=$1`, req.user.workspaceId).then((r) => r[0].count),
            ]);
    ...
    router.delete('/signal-log/:id', async (req, res) => {
        try {
            await db.$executeRawUnsafe(`DELETE FROM "SignalLog" WHERE "id"=$1 AND "workspaceId"=$2`, req.params.id, req.user.workspaceId);
            res.json({ ok: true });
        }
        catch (err) {
