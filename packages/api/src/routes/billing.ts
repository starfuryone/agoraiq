...
export function createBillingRoutes(db: PrismaClient): Router {
  const router = Router();
  ...
  const stripe = stripeKey ? new Stripe(stripeKey, { apiVersion: '2024-12-18' as any }) : null as any;
...
async function processWebhookEvent(db: PrismaClient, stripe: Stripe, event: Stripe.Event) {
  // Idempotency check
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed':
        await handleCheckoutCompleted(db, stripe, event.data.object as any);
        break;
      case 'invoice.paid':
        await handleInvoicePaid(db, event.data.object as any);
        break;
      ...
      case 'invoice.payment_failed':
        await handlePaymentFailed(db, event.data.object as any);
        break;
      default:
...
async function handleCheckoutCompleted(db: PrismaClient, stripe: Stripe, session: any) {
...
async function handleInvoicePaid(db: PrismaClient, invoice: any) {
...
async function handleSubscriptionUpdated(db: PrismaClient, stripeSub: any) {
...
async function handleSubscriptionDeleted(db: PrismaClient, stripeSub: any) {
  const sub = await db.subscription.findFirst({ where: { stripeSubscriptionId: stripeSub.id } });
  if (!sub) return;
...
async function handlePaymentFailed(db: PrismaClient, invoice: any) {
  const subscriptionId = invoice.subscription as string;
  if (!subscriptionId) return;
