...
const log = (0, db_1.createLogger)('marketplace');
function createMarketplaceRoutes(db) {
    const router = (0, express_1.Router)();
    // ── GET /providers ────────────────────────────────────────
    router.get('/providers', async (req, res) => {
        try {
            const { market, style, leverage, exchange, language, sort = 'volAdj', minTrades = '0', limit = '50', offset = '0', } = req.query;
            // Build filter
            const where = {
            ...
            if (tierFilter === 'VERIFIED')
                where.marketplaceTier = 'VERIFIED';
            else if (tierFilter === 'BETA')
                where.marketplaceTier = 'BETA';
            else if (tierFilter === 'PENDING')
                where.marketplaceTier = 'PENDING';
            else
                where.marketplaceTier = { in: ['VERIFIED', 'BETA', 'PENDING'] };
            // Search filter
            ...
            if (typeFilter === 'SIGNAL')
                where.providerType = 'SIGNAL';
            if (typeFilter === 'TOOL')
                where.providerType = 'TOOL';
            if (typeFilter === 'ALERT')
                where.providerType = 'ALERT';
            if (market && market !== 'All')
                where.marketType = market;
            if (style && style !== 'All')
                where.tradingStyle = style;
            if (leverage && leverage !== 'All')
                where.leverageBand = leverage;
            if (exchange && exchange !== 'All')
                where.exchangeFocus = exchange;
    ...
    router.get('/providers/:slug/monthly', async (req, res) => {
        try {
            const provider = await db.provider.findUnique({
                where: { slug: String(req.params.slug) },
            ...
            const months = await db.providerMonthlyStats.findMany({
    ...
    router.get('/providers/:slug/timeline', async (req, res) => {
        try {
            const provider = await db.provider.findUnique({
                where: { slug: String(req.params.slug) },
                select: { id: true, marketplaceVisible: true },
    ...
    router.post('/import', async (req, res) => {
        try {
            ...
            const currentImports = await db.workspaceProviderImport.count({
            ...
            // Upsert import
            const imported = await db.workspaceProviderImport.upsert({
                where: {
    ...
    router.post('/submit', async (req, res) => {
        try {
            ...
            // Dedupe check
            const existing = await db.providerSubmission.findFirst({
            ...
            const user = req.user;
            const submission = await db.providerSubmission.create({
                data: {
                    channelName,
                    channelUrl,
    ...
    router.get('/submit/pending', async (req, res) => {
        try {
            const user = req.user;
            if (!user) {
                res.status(401).json({ error: 'AUTH_REQUIRED' });
            ...
            const submissions = await db.providerSubmission.findMany({
    ...
    router.get('/tiers/summary', async (_req, res) => {
        try {
            const [verified, beta, pending] = await Promise.all([
                ...
                db.providerSubmission.count({ where: { status: 'PENDING' } }),
            ]);
    ...
    router.get('/summary', async (_req, res) => {
        try {
            ...
            const trades30d = await db.$queryRawUnsafe(`SELECT COALESCE(SUM(trade_count),0)::int AS total FROM provider_stats_snapshot WHERE period = '30d'`);
            const tradesAll = await db.$queryRawUnsafe(`SELECT COALESCE(SUM(trade_count),0)::int AS total FROM provider_stats_snapshot WHERE period = 'all'`);
            res.json({
