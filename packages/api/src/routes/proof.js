...
exports.createProofRoutes = createProofRoutes;
const express_1 = require("express");
const db_1 = require("@agoraiq/db");
const cache_1 = require("../services/cache");
const safe_mode_1 = require("../services/safe-mode");
const rate_limit_1 = require("../middleware/rate-limit");
const log = (0, db_1.createLogger)('proof-routes');
...
// ── Route Factory ─────────────────────────────────────────────
function createProofRoutes(db, defaultWorkspaceId) {
    const router = (0, express_1.Router)();
    // Apply rate limiting to ALL proof endpoints
    router.use(rate_limit_1.proofRateLimiter);
    // ── GET /stats ──────────────────────────────────────────────
    router.get('/stats', async (req, res) => {
        try {
            const category = req.query.category || 'all';
            const cacheKey = `proof:stats:${category}`;
            const cached = cache_1.proofCache.get(cacheKey);
            if (cached) {
                res.json(cached);
                return;
            }
            ...
            const closedStatuses = ['HIT_SL', 'HIT_TP', 'EXPIRED'];
            ...
            (0, safe_mode_1.assertSafe)(stats);
            cache_1.proofCache.set(cacheKey, stats);
            res.json(stats);
        }
        catch (err) {
    ...
    router.get('/monthly', async (req, res) => {
        try {
            const category = req.query.category || 'all';
            const monthsBack = Math.min(parseInt(req.query.months, 10) || 6, safe_mode_1.SAFE_MODE_CONFIG.maxMonths);
            ...
            (0, safe_mode_1.assertSafe)(result);
            cache_1.proofCache.set(cacheKey, result);
            res.json(result);
        }
        catch (err) {
    ...
    router.get('/feed', async (req, res) => {
        try {
            ...
            const rankings = providerStats
                ...
                .map((p, i) => ({ ...p, rank: i + 1 }));
            const providerMask = (0, safe_mode_1.buildProviderMask)(rankings);
            // Apply safe-mode feed transformation
            const safeFeed = (0, safe_mode_1.safeModeFeed)(tradesWithMeta.map(t => ({ ...t, createdAt: t.createdAt })), providerMask);
    ...
    router.get('/stream', rate_limit_1.sseConnectionGuard, (req, res) => {
        res.writeHead(200, {
