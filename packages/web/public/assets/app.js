/* ═══════════════════════════════════════════════════════════════
   ...
   Auth: Bearer token stored in localStorage as 'iq_token'
═══════════════════════════════════════════════════════════════ */
...
// ── Set header username from token payload ────────────────────
function loadHeaderUser() {
  const badge = document.getElementById('username-badge');
  if (!badge) return;
  // Try to decode JWT payload for display name (no verify, display only)
  const token = getToken();
  if (!token) return;
  try {
    const payload = JSON.parse(atob(token.split('.')[1]));
    badge.textContent = payload.name || payload.email?.split('@')[0] || 'User';
  } catch (_) {}
}
