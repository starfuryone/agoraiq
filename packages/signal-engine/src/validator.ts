  ...
  validate(signal: Partial<SignalFields>): ValidationResult {
    ...
    if (!signal.symbol || signal.symbol.trim() === '') {
      ...
      errors.push({
        ...
        code: 'INVALID_FORMAT',
        message:
          'Symbol must be uppercase with no slash (e.g. BTCUSDT not BTC/USDT)',
      })
    }
    ...
    if (!signal.exchange || signal.exchange.trim() === '') {
      warnings.push({
        field: 'exchange',
        message:
          'No exchange specified — Cornix users may not know where to execute',
      })
    }

    // ── Entries ────────────────────────────────────────────────────────────
    const entries = signal.entries as number[] | undefined
    if (!entries || entries.length === 0) {
      ...
      if (
        ...
        errors.push({
          ...
          code: 'INVALID',
          message:
            'Entry range must be [lower, upper] — first value must be less than second',
        })
      }
    }
    ...
    if (signal.stopLoss === undefined || signal.stopLoss === null) {
      errors.push({
        ...
        code: 'MISSING',
        message:
          'Stop Loss is required — signals cannot be published without a stop loss',
      })
    ...
    if (!signal.targets || signal.targets.length === 0) {
      ...
      const invalidTarget = signal.targets.find(
      ...
      if (signal.targets.length === 1) {
        warnings.push({
          field: 'targets',
          message:
            'Only 1 TP set — consider adding TP2/TP3 for better Cornix compatibility and risk management',
        })
      }
    }
    ...
    if (signal.leverage !== undefined && signal.leverage !== '') {
      if (!LEVERAGE_PATTERN.test(signal.leverage)) {
        errors.push({
          ...
          code: 'INVALID_FORMAT',
          message:
            'Leverage must use Cornix format: "Cross 10x" or "Isolated 5x"',
        })
      }
    } else {
      warnings.push({
        field: 'leverage',
        message:
          'No leverage specified — Cornix will use its own default setting',
      })
    }
    ...
    const hasEntries =
      signal.entries && signal.entries.length > 0 && signal.entries.every((e) => e > 0)
    const hasSL =
    ...
    if (hasEntries && hasSL && hasTargets && hasDirection) {
      const avgEntry =
        signal.entries!.reduce((sum, e) => sum + e, 0) / signal.entries!.length
