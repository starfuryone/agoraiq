...
import { CornixParseResult, CornixSignal, Direction, MarketType } from './cornix'
...
function validateResponse(raw: unknown): LLMSignalResponse | null {
  ...
  const marketType = ['SPOT', 'FUTURES_PERP', 'FUTURES_DATED'].includes(r.marketType as string)
    ? r.marketType as MarketType : null
  return {
    ...
    takeProfits:  Array.isArray(r.takeProfits)
                    ? (r.takeProfits as unknown[]).filter(n => typeof n === 'number') as number[]
                    : [],
...
const PROVIDERS: Record<LLMProvider, (prompt: string) => Promise<string>> = {
...
export async function parseLLM(
  raw: string,
  options?: { promptTemplate?: string | null; promptVersion?: number; providers?: LLMProvider[] }
): Promise<CornixParseResult & { provider?: LLMProvider }> {
