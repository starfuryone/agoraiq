 ...
 */
export function assertCornixTelegramOptions(opts: TelegramSendOptions): void {
  if (opts.parse_mode !== undefined) {
    throw new Error(
      `Cornix Telegram posts must be plain text — do not set parse_mode.\n` +
 ...
 */
export function cornixTelegramOptions(
  extras?: Omit<TelegramSendOptions, 'parse_mode'>,
): TelegramSendOptions {
 ...
 * Invalid: ```json\nTP1 - 96000\n```
 */
export function assertUntypedCodeFence(content: string): void {
  if (TYPED_FENCE_RE.test(content)) {
    const tag = content.match(/^```([a-zA-Z]+)\n/)?.[1] ?? 'unknown'
    throw new Error(
      `Discord Cornix content must use an untyped code fence.\n` +
 ...
 */
export function buildDiscordCornixPayload(
  cornixText: string,
  embed?: DiscordEmbed,
