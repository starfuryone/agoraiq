...
exports.broadcastSignalAlert = broadcastSignalAlert;
const telegraf_1 = require("telegraf");
const db_1 = require("@agoraiq/db");
const log = (0, db_1.createLogger)('telegram-bot');
...
async function getTelegramUser(telegramId) {
    return db_1.db.telegramUser.findUnique({
        where: { telegramId },
        include: {
            user: {
...
async function requirePaid(ctx) {
    ...
    if (!tgUser) {
        await ctx.reply('🔒 You need to link your AgoraIQ account first.\n\n' +
            'Use /start to begin onboarding.');
        return false;
    }
    if (!tgUser.user.subscription || tgUser.user.subscription.status !== 'active') {
        await ctx.reply('🔒 Active subscription required.\n\n' +
...
bot.start(async (ctx) => {
    ...
    if (existing) {
        await ctx.reply(`✅ Welcome back! You're linked as ${existing.user.email}.\n\n` +
            'Commands:\n' +
    ...
    await ctx.reply('👋 Welcome to AgoraIQ Signals!\n\n' +
...
bot.command('link', async (ctx) => {
    ...
    try {
        ...
        await db_1.db.telegramUser.create({
            data: {
                telegramId,
                userId: user.id,
        ...
        await ctx.reply(`✅ Linked to ${email}! Your credentials message has been deleted for security.\n\n` +
...
// ── /help ─────────────────────────────────────────────────────
bot.help(async (ctx) => {
    await ctx.reply('📋 AgoraIQ Commands:\n\n' +
...
bot.command('signals', async (ctx) => {
    ...
    if (!tgUser)
        return;
    try {
        const signals = await db_1.db.signal.findMany({
...
bot.command('watch', async (ctx) => {
    ...
    try {
        await db_1.db.watchlist.upsert({
...
bot.command('unwatch', async (ctx) => {
    ...
    try {
        await db_1.db.watchlist.updateMany({
...
bot.command('watchprovider', async (ctx) => {
    ...
    if (!slug) {
        await ctx.reply('Usage: /watchprovider itb');
        return;
    }
    try {
        await db_1.db.watchlist.upsert({
...
bot.command('providers', async (ctx) => {
    if (!(await requirePaid(ctx)))
        return;
    try {
        const providers = await db_1.db.provider.findMany({ where: { isActive: true } });
        const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 3600_000);
        const board = await Promise.all(providers.map(async (p) => {
            const trades = await db_1.db.trade.findMany({
                where: {
                    providerId: p.id,
        ...
        const lines = board.map((p, i) => `${i + 1}. ${p.name} — WR: ${p.winRate}% | Avg R: ${p.avgR.toFixed(2)} | Trades: ${p.total}`);
...
bot.command('provider', async (ctx) => {
    ...
    const slug = ctx.message.text.split(' ')[1]?.toLowerCase();
    if (!slug) {
        await ctx.reply('Usage: /provider itb');
        return;
    }
    try {
        const provider = await db_1.db.provider.findUnique({ where: { slug } });
        if (!provider) {
            await ctx.reply('❌ Provider not found.');
            return;
        }
        const recent = await db_1.db.signal.findMany({
...
bot.command('mute', async (ctx) => {
    const tgUser = await getTelegramUser(ctx.from.id.toString());
    if (!tgUser) {
        await ctx.reply('Not linked. Use /start');
        return;
    }
    await db_1.db.telegramUser.update({ where: { id: tgUser.id }, data: { muteAll: true } });
...
bot.command('unmute', async (ctx) => {
    ...
    await db_1.db.telegramUser.update({ where: { id: tgUser.id }, data: { muteAll: false } });
...
bot.command('digest', async (ctx) => {
    ...
    await db_1.db.telegramUser.update({ where: { id: tgUser.id }, data: { digestEnabled: enabled } });
...
bot.command('status', async (ctx) => {
    ...
    const watchlists = await db_1.db.watchlist.findMany({
    ...
    await ctx.reply(`👤 ${tgUser.user.email}\n` +
...
async function broadcastSignalAlert(signal) {
    try {
        ...
        const watchingSymbol = await db_1.db.watchlist.findMany({
        ...
        const watchingProvider = await db_1.db.watchlist.findMany({
