...
async function emitToFeed(trade, status, rMultiple) {
    try {
        const provider = await db_1.db.provider.findUnique({
...
const log = (0, db_1.createLogger)('tracker-worker');
const POLL_INTERVAL = parseInt(process.env.TRACKER_POLL_INTERVAL_MS || '30000', 10);
const BATCH_SIZE = 50;
// ── R-Multiple Calculation ────────────────────────────────────
function calculateRMultiple(direction, entryPrice, exitPrice, slPrice) {
    if (direction === 'LONG') {
...
function calculatePnlPct(direction, entryPrice, exitPrice, leverage) {
...
async function resolveTrade(trade) {
    ...
    const result = await (0, price_service_1.checkPriceHit)(trade.symbol, trade.exchange, trade.direction, trade.tpPrice, trade.slPrice);
    if (!result) {
    ...
    if (result.hitTP) {
        const now2 = new Date();
        const tpLevels = [
            { field: 'tp1Price', hitField: 'tp1HitAt', price: trade.tp1Price, hitAt: trade.tp1HitAt },
            { field: 'tp2Price', hitField: 'tp2HitAt', price: trade.tp2Price, hitAt: trade.tp2HitAt },
            { field: 'tp3Price', hitField: 'tp3HitAt', price: trade.tp3Price, hitAt: trade.tp3HitAt },
        ...
        if (newHits > 0) {
            ...
            if (allHit) {
                ...
                await db_1.db.trade.update({ where: { id: trade.id }, data: updateData });
...
// ── Audit Helper ──────────────────────────────────────────────
async function auditTradeResolution(tradeId, status, providerId) {
    try {
        ...
        if (status === 'HIT_TP' || status === 'HIT_SL') {
            const trade = await db_1.db.trade.findUnique({
                where: { id: tradeId },
...
async function pollActiveTrades() {
    try {
        const activeTrades = await db_1.db.trade.findMany({
            where: { status: 'ACTIVE' },
