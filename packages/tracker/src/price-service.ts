...
function resolveProxyConfig(): { url: string; enabled: boolean } {
...
function getSocksAgent(): SocksProxyAgent | null {
  ...
  if (!_socksAgent) {
    try {
      _socksAgent = new SocksProxyAgent(PROXY_CONFIG.url);
      ...
      log.error({ err: (err as Error).message }, 'Failed to create SOCKS5 agent — proxy disabled');
      return null;
    }
  }
  return _socksAgent;
...
function mapSymbol(symbol: string, exchange: string): string {
  ...
  if (exchange === 'COINBASE') {
    ...
    if (coinbaseMap[sym]) return coinbaseMap[sym];
...
async function fetchWithRetry(
  ...
  const urlHost = new URL(url).hostname; // safe to log (no credentials)
  ...
  if (useProxy) {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (isLastRetry) {
          ...
          log.warn(
            { host: urlHost, attempts: retries },
            'All proxy attempts failed — proxy may be down',
          );
        } else {
          const wait = delay * Math.pow(2, i);
          log.warn(
            { attempt: i + 1, host: urlHost, wait, error: err?.message },
            'Price fetch retry (proxy)',
          );
    ...
    if (PROXY_FALLBACK_DIRECT) {
      ...
      try {
        ...
        log.error(
          { host: urlHost, error: err?.message },
...
export async function getCurrentPrice(
  ...
  try {
    ...
    // ── Kraken REST (always direct — no proxy) ───────────
      ...
      data = await fetchWithRetry(
        `${COINBASE_API}/products/${mappedSymbol}/candles?granularity=60`,
        false,
      );
