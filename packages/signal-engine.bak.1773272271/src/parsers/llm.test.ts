...
async function run() {
  ...
  const r1 = await parseSignal(FREEFORM_1, {
    mode: 'FREEFORM',
  ...
  const r2 = await parseSignal(FREEFORM_2, {
    mode: 'FREEFORM',
    confidenceThreshold: 0.6,
  })
  ...
  const r3 = await parseSignal(CORNIX_LOW_CONFIDENCE, {
    mode: 'CORNIX',
