...
exports.parseCornix = parseCornix;
function normalisePair(raw) {
    return (raw || '').replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
}
function parseEntryZone(raw) {
...
function parseTakeProfits(lines) {
    const tps = [];
...
function detectMarketType(text) {
    ...
    if (lower.includes('futures') || lower.includes('perp') || lower.includes('swap'))
        return 'FUTURES_PERP';
    if (lower.includes('leverage') || lower.includes('cross') || lower.includes('isolated'))
        return 'FUTURES_PERP';
    return 'SPOT';
}
function sanityCheck(signal) {
    const errors = [];
    if (!signal.pair)
        errors.push('Missing pair');
    if (!signal.stopLoss)
        errors.push('Missing stop loss');
    if (signal.takeProfits.length === 0)
        errors.push('No take profits found');
    ...
    if (signal.takeProfits.length > 1) {
        for (let i = 1; i < signal.takeProfits.length; i++) {
            ...
            if (!expected) {
                errors.push('TP levels are not in correct order');
                break;
            }
        }
    }
...
function scoreConfidence(signal, errors) {
    let score = 1.0;
    if (!signal.pair)
        score -= 0.3;
    if (!signal.entryMin)
        score -= 0.05; // market order — soft penalty only
    if (!signal.stopLoss)
        score -= 0.2;
    if (signal.takeProfits.length === 0)
        score -= 0.2;
...
//# sourceMappingURL=cornix.js.map