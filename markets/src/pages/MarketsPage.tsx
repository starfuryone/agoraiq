...
import {
  DEFAULT_COLUMNS, STATUS_COLOR, STATUS_DOT, SCORE_COLOR,
  fmt, generateMockRows, Cell,
  API_BASE, WS_URL,
} from './markets-utils';

import type {
  MarketRow, FilterState, SortSpec, ColumnDef,
  CompareRow, ChangelogRow, SyncRow, ExchangeMeta,
} from '../types/market';
...
const Icon = {
  ...
  Sort:     (d: 'asc'|'desc') => <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24">{d==='asc' ? <path d="M12 5l7 7H5z"/> : <path d="M12 19l-7-7h14z"/>}</svg>,
  Chevron:  (open: boolean) => <svg className={`w-3.5 h-3.5 transition-transform ${open?'rotate-180':''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg>,
...
function TopBar({
  filters, onFilter, live, onToggleLive, onOpenColumns,
  onExport, onOpenExchanges, totalCount, visibleCount,
}: {
  filters: FilterState;
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'e' || e.key === 'E') {
        if (document.activeElement?.tagName !== 'INPUT') onOpenExchanges();
      }
      if (e.key === 'c' || e.key === 'C') {
        if (document.activeElement?.tagName !== 'INPUT') onOpenColumns();
      }
    };
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 bg-[#0d0e10] border-b border-[#1e2025] flex-wrap">
      {/* Search */}
      <div className="relative flex items-center">
        ...
        <input
          ...
          placeholder="Search pairs, assets… (/)"
          className="ml-1.5 bg-[#161820] border border-[#2a2d35] rounded px-2 py-1 text-xs text-zinc-200 placeholder-zinc-600 focus:outline-none focus:border-blue-500 w-52"
        />
        {filters.search && (
          <button
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-2.5 py-1 bg-[#161820] border border-[#2a2d35] rounded text-xs text-zinc-400 hover:text-zinc-200 hover:border-zinc-500 transition-colors"
      >
        <span>Exchanges</span>
      ...
      <div className="flex items-center gap-1">
        {STATUS_OPTIONS.map(s => {
          const active = filters.status.includes(s as any);
          const color = active ? STATUS_COLOR[s] : 'text-zinc-600 bg-transparent';
          return (
            <button
              key={s}
              onClick={() => {
              ...
              className={`px-1.5 py-0.5 rounded text-[10px] font-medium border transition-all ${
                active
                  ? `${STATUS_COLOR[s]} border-current/30`
...
function LeftRail({
  open, filters, onFilter, exchanges,
}: {
  open: boolean;
  filters: FilterState;
  onFilter: (f: Partial<FilterState>) => void;
  ...
  return (
    <div className="w-48 flex-shrink-0 bg-[#0d0e10] border-r border-[#1e2025] overflow-y-auto flex flex-col text-xs">
      ...
      <div className="px-3 py-2 border-b border-[#1e2025] flex items-center justify-between">
        ...
        {(filters.exchanges.length > 0 || filters.bases.length > 0 || filters.quotes.length > 0 || filters.marginAvailable) && (
          <button
            onClick={() => onFilter({ exchanges: [], bases: [], quotes: [], marginAvailable: false, minVolume: null, maxSpreadBps: null })}
      ...
      <div className="border-b border-[#1e2025]">
        <button
          ...
          className="w-full flex items-center justify-between px-3 py-2 text-zinc-400 hover:text-zinc-200"
        >
          <span className="font-medium">Exchanges</span>
          <div className="flex items-center gap-1">
            {filters.exchanges.length > 0 && (
              <span className="text-[9px] text-blue-400 font-medium">{filters.exchanges.length}</span>
            )}
            {Icon.Chevron(exOpen)}
          </div>
        </button>
        ...
        {exOpen && (
          <div className="pb-2 px-2">
            <div className="flex gap-1 mb-1.5">
              <button
                onClick={() => onFilter({ exchanges: [] })}
                className="px-1.5 py-0.5 text-[9px] bg-[#161820] border border-[#2a2d35] rounded text-zinc-500 hover:text-zinc-300"
              >All</button>
              <button
                onClick={() => {
            ...
            {exchanges.map(ex => (
              <label key={ex.exchange} className="flex items-center gap-2 py-0.5 cursor-pointer hover:bg-[#161820] px-1 rounded">
                <input
                  type="checkbox"
                  checked={filters.exchanges.includes(ex.exchange)}
                  onChange={() => toggleEx(ex.exchange)}
      ...
      <div className="border-b border-[#1e2025]">
        ...
        {baseOpen && (
          <div className="pb-2 px-2 flex flex-wrap gap-1">
            {TOP_BASES.map(b => (
              <button
                key={b}
                onClick={() => toggleBase(b)}
                className={`px-2 py-0.5 text-[10px] rounded border font-medium transition-colors ${
                  filters.bases.includes(b)
                    ? 'bg-blue-500/20 border-blue-500/40 text-blue-300'
                    : 'bg-[#161820] border-[#2a2d35] text-zinc-500 hover:text-zinc-300'
                }`}
              >{b}</button>
            ))}
          </div>
        ...
        {advOpen && (
          <div className="pb-3 px-3 space-y-3">
            ...
            <label className="flex items-center justify-between cursor-pointer">
              ...
              <div
                onClick={() => onFilter({ marginAvailable: !filters.marginAvailable })}
                className={`w-8 h-4 rounded-full transition-colors relative cursor-pointer ${filters.marginAvailable ? 'bg-blue-500' : 'bg-zinc-700'}`}
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={e => onFilter({ minVolume: e.target.value ? parseFloat(e.target.value) : null })}
                placeholder="e.g. 100000"
                className="w-full bg-[#161820] border border-[#2a2d35] rounded px-2 py-1 text-[11px] text-zinc-300 font-mono focus:outline-none focus:border-blue-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="number"
                value={filters.maxSpreadBps ?? ''}
                onChange={e => onFilter({ maxSpreadBps: e.target.value ? parseFloat(e.target.value) : null })}
                placeholder="e.g. 50"
                className="w-full bg-[#161820] border border-[#2a2d35] rounded px-2 py-1 text-[11px] text-zinc-300 font-mono focus:outline-none focus:border-blue-500"
              />
            </div>
...
function ColumnPicker({
  columns, onClose, onChange,
}: {
  columns: ColumnDef[];
  onClose: () => void;
  onChange: (cols: ColumnDef[]) => void;
}) {
  const groups = ['identity', 'live', 'specs', 'ops'] as const;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[#13141a] border border-[#2a2d35] rounded-lg shadow-2xl w-80">
          ...
          {groups.map(group => (
            <div key={group}>
              ...
              {columns.filter(c => c.group === group).map(col => (
                <label key={col.key} className="flex items-center gap-2 py-1 cursor-pointer hover:bg-[#1e2025] px-2 rounded">
                  <input
                    type="checkbox"
                    checked={col.visible}
                    onChange={() => onChange(columns.map(c => c.key === col.key ? { ...c, visible: !c.visible } : c))}
        ...
        <div className="px-4 py-3 border-t border-[#2a2d35] flex justify-end">
          <button
            onClick={() => onChange(columns.map(c => ({ ...c, visible: DEFAULT_COLUMNS.find(d => d.key === c.key)?.visible ?? true })))}
...
function Inspector({
  ...
  const [tab, setTab] = useState<'overview'|'compare'|'changelog'|'sync'|'raw'>('overview');
  const [compareData, setCompareData] = useState<CompareRow[]>([]);
  const [changelog, setChangelog] = useState<ChangelogRow[]>([]);
  const [syncHistory, setSyncHistory] = useState<SyncRow[]>([]);
  ...
  useEffect(() => {
    if (tab === 'compare') {
      setLoading(true);
      fetch(`${API_BASE}/markets/compare?base=${row.baseCanonical}&quote=${row.quoteCanonical}`)
        .then(r => r.json())
      ...
      setLoading(true);
      fetch(`${API_BASE}/markets/${row.exchange}/${row.pairId}`)
        .then(r => r.json())
  ...
  return (
    <div
      className="bg-[#0d0e10] border-t border-[#1e2025] flex flex-col flex-shrink-0"
      style={{ height }}
    >
      {/* Resize handle */}
      <div
        onMouseDown={handleResizeMouseDown}
        className="h-1 bg-transparent hover:bg-blue-500/30 cursor-ns-resize transition-colors flex-shrink-0"
      />
      ...
      <div className="flex-1 overflow-auto">
          ...
          <div className="flex gap-0 h-full">
            ...
            <div className="w-64 border-r border-[#1e2025] p-4 flex-shrink-0">
              ...
              <div className="space-y-1.5">
                {[
                  ['Status',     <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${(STATUS_COLOR as any)[row.status] || ''}`}>{row.status}</span>],
                  ...
                  ['Last Sync',  <span className="text-zinc-400 text-xs">{fmt.relTime(row.lastSyncedAt)}</span>],
                ].map(([label, val]) => (
              ...
              <div className="mt-4 space-y-1.5">
                {[
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-between px-2 py-1.5 bg-[#161820] border border-[#2a2d35] rounded text-[10px] text-zinc-500 hover:text-zinc-300 hover:border-zinc-500 transition-colors"
                  >
            ...
            <div className="flex-1 p-4">
              ...
              <div className="grid grid-cols-4 gap-3">
                {[
                  { label: 'Last Price',   val: fmt.price(row.last),     accent: 'text-white', mono: true },
                  { label: 'Bid',          val: fmt.price(row.bid),      accent: 'text-emerald-400', mono: true },
                  { label: 'Ask',          val: fmt.price(row.ask),      accent: 'text-red-400', mono: true },
                  { label: 'Spread',       val: fmt.price(row.spreadAbs), accent: 'text-zinc-300', mono: true },
                  { label: 'Spread (bps)', val: fmt.bps(row.spreadBps) + ' bps', accent: row.spreadBps !== null && row.spreadBps < 5 ? 'text-emerald-400' : 'text-amber-400', mono: true },
                  { label: 'Vol 24h',      val: fmt.vol(row.volume24h),  accent: 'text-zinc-300', mono: true },
                  { label: 'Vol USD',      val: '$' + fmt.vol(row.volume24hUsd), accent: 'text-zinc-400', mono: true },
                  { label: 'Funding',      val: fmt.funding(row.fundingRate), accent: row.fundingRate !== null && row.fundingRate >= 0 ? 'text-emerald-400' : 'text-red-400', mono: true },
                ].map(({ label, val, accent, mono }) => (
                  <div key={label} className="bg-[#161820] border border-[#2a2d35] rounded p-3">
        ...
        {tab === 'compare' && (
          <div className="p-3">
            ...
            {loading ? (
              <div className="text-zinc-600 text-xs py-8 text-center">Loading cross-exchange data…</div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-[10px] text-zinc-600 border-b border-[#1e2025]">
                    {['Exchange','Price','Spread bps','Vol 24h','Liquidity','Status','Best'].map(h => (
                ...
                <tbody>
                  {compareData.map((r, i) => (
                    <tr
                      key={r.exchange}
                      className={`border-b border-[#1e2025]/50 hover:bg-[#161820] ${r.exchange === row.exchange ? 'bg-blue-500/5' : ''}`}
                    >
                      ...
                      <td className="py-1.5 px-3 font-mono text-zinc-200">{fmt.price(r.last)}</td>
                      <td className={`py-1.5 px-3 font-mono ${r.isBestSpread ? 'text-emerald-400 font-semibold' : 'text-zinc-400'}`}>{fmt.bps(r.spreadBps)}</td>
                      <td className="py-1.5 px-3 font-mono text-zinc-300">{fmt.vol(r.volume24h)}</td>
                      <td className="py-1.5 px-3">
                        {r.liquidityScore !== null
                          ? <span className={SCORE_COLOR(r.liquidityScore)}>{r.liquidityScore}</span>
                          : <span className="text-zinc-700">—</span>}
                      </td>
                      <td className="py-1.5 px-3">
                        <span className={`px-1.5 py-0.5 rounded text-[9px] font-medium ${(STATUS_COLOR as any)[r.status] || ''}`}>{r.status}</span>
                      </td>
                      <td className="py-1.5 px-3">
        ...
        {tab === 'changelog' && (
          <div className="p-3">
            ...
            {loading ? (
              ...
              <div className="text-zinc-600 text-xs py-8 text-center">No changes recorded for this pair.</div>
            ) : (
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-[10px] text-zinc-600 border-b border-[#1e2025]">
                    {['Type','Field','Old','New','Detected'].map(h => (
                      <th key={h} className="py-2 px-3 text-left font-medium">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {changelog.map(c => (
                    <tr key={c.id} className="border-b border-[#1e2025]/50 hover:bg-[#161820]">
                      <td className="py-1.5 px-3">
                        <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${
                          c.changeType === 'DELIST' ? 'text-red-400 bg-red-400/10' :
                      ...
                      <td className="py-1.5 px-3 text-zinc-500">{fmt.relTime(c.detectedAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {tab === 'sync' && (
          <div className="p-3">
            ...
            {loading ? (
              ...
              <div className="text-zinc-600 text-xs py-8 text-center">No sync history available.</div>
            ) : (
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-[10px] text-zinc-600 border-b border-[#1e2025]">
                    {['Status','Fetched','Upserted','Delisted','Changes','Duration','Started'].map(h => (
                      <th key={h} className="py-2 px-3 text-left font-medium">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {syncHistory.map(s => (
                    <tr key={s.id} className="border-b border-[#1e2025]/50 hover:bg-[#161820]">
                      <td className="py-1.5 px-3">
                        <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${
                          s.status === 'SUCCESS' ? 'text-emerald-400 bg-emerald-400/10' :
                          s.status === 'FAILED'  ? 'text-red-400 bg-red-400/10' :
                          'text-amber-400 bg-amber-400/10'
                        }`}>{s.status}</span>
                        {s.errorMessage && (
                          <span className="block text-[9px] text-red-400 mt-0.5">{s.errorMessage.slice(0, 40)}</span>
                        )}
                      </td>
                      ...
                      <td className="py-1.5 px-3 text-zinc-500">{fmt.relTime(s.startedAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function MarketGrid({
  rows, columns, sort, onSort, selectedId, onSelect,
}: {
  rows:       MarketRow[];
  columns:    ColumnDef[];
  sort:       SortSpec[];
  ...
  return (
    <div ref={containerRef} className="flex-1 overflow-auto relative" onScroll={handleScroll}>
      <div style={{ height: totalHeight + ROW_HEIGHT, minWidth: totalWidth }} className="relative">
        ...
        {/* Sticky header */}
        <div
          className="sticky top-0 z-10 flex bg-[#0d0e10] border-b border-[#1e2025]"
          ...
          {visibleCols.map(col => {
            ...
            return (
              <div
                ...
                className={`flex items-center gap-1 px-3 py-2 text-[10px] font-medium text-zinc-500 tracking-wider uppercase select-none flex-shrink-0 ${
                  col.sortable ? 'cursor-pointer hover:text-zinc-300 hover:bg-[#161820]' : ''
                } ${col.align === 'right' ? 'justify-end' : col.align === 'center' ? 'justify-center' : 'justify-start'}`}
                onClick={() => col.sortable && onSort(col.key)}
              >
                <span>{col.label}</span>
                {col.sortable && sortEntry && (
                  <span className="text-blue-400">{Icon.Sort(sortEntry.dir)}</span>
                )}
              </div>
            );
          })}
        ...
        {visibleRows.map((row) => {
          ...
          return (
            <div
              ...
              className={`flex items-center cursor-pointer border-b border-[#1e2025]/50 transition-colors flex-shrink-0 ${
                selected
            ...
            >
              {visibleCols.map(col => (
                <div
                  key={col.key}
                  style={{ width: col.width, minWidth: col.width }}
                  className={`px-3 flex items-center flex-shrink-0 overflow-hidden ${
                    col.align === 'right' ? 'justify-end' : col.align === 'center' ? 'justify-center' : 'justify-start'
                  }`}
                >
...
export default function MarketsPage() {
  ...
  const loadMarkets = useCallback(async () => {
    ...
    try {
      ...
      if (filters.bases.length)     params.set('base', filters.bases[0]);
      if (filters.quotes.length)    params.set('quote', filters.quotes[0]);
      if (filters.status.length)    params.set('status', filters.status.join(','));
      if (filters.marginAvailable)  params.set('marginAvailable', 'true');
      if (filters.minVolume)        params.set('minVolume', String(filters.minVolume));
      if (filters.maxSpreadBps)     params.set('maxSpreadBps', String(filters.maxSpreadBps));
      ...
      setAllRows(generateMockRows(400));
    } finally {
      setLoading(false);
    }
  }, [filters, sort]);
  ...
  useEffect(() => {
    fetch(`${API_BASE}/markets/meta/exchanges`)
      .then(r => r.json())
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0a0b0d] text-zinc-300 overflow-hidden" style={{ fontFamily: "'IBM Plex Sans', 'JetBrains Mono', monospace" }}>
      ...
      <TopBar
        ...
        onOpenColumns={() => setColumnPickerOpen(true)}
        onExport={exportCSV}
        onOpenExchanges={() => setExchangePickerOpen(true)}
        ...
        <div className="flex flex-col flex-1 min-w-0 min-h-0">
          ...
          {selectedRow && (
            <Inspector
              ...
              onResize={(h) => setInspectorHeight(Math.max(180, Math.min(600, h)))}
            />
          )}
        </div>
      </div>
          ...
          <div className="bg-[#13141a] border border-[#2a2d35] rounded-lg shadow-2xl w-80">
            ...
            <div className="p-3 space-y-0.5 max-h-[400px] overflow-y-auto">
              <div className="flex gap-2 mb-3 px-1">
                <button
                  onClick={() => setFilters(prev => ({ ...prev, exchanges: [] }))}
                  className="text-[10px] text-zinc-500 hover:text-zinc-300 border border-zinc-700 px-2 py-1 rounded"
              ...
              {[1, 2, 3].map(tier => (
                <div key={tier}>
                  ...
                  {exchanges.filter(e => e.tier === tier).map(ex => {
                    ...
                    return (
                      <label key={ex.exchange} className="flex items-center gap-2.5 px-2 py-1.5 hover:bg-[#1e2025] rounded cursor-pointer">
                        <input
                          type="checkbox"
                          checked={active || filters.exchanges.length === 0}
                          onChange={() => {
                        ...
                        {ex.totalPairs > 0 && (
                          <span className="ml-auto text-[10px] text-zinc-600 font-mono">{ex.totalPairs.toLocaleString()}</span>
                        )}
                      </label>
                    );
                  })}
            ...
            <div className="px-4 py-3 border-t border-[#2a2d35] flex justify-end gap-2">
              ...
              >Clear</button>
              <button
                onClick={() => setExchangePickerOpen(false)}
